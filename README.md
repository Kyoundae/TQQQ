...
| `.github/workflows/daily-signal.yml` | 매일 새벽 실행 (약 05:47 KST 예약, 위 전부 포함) |
...
- 이후로는 매일 새벽(약 05:47 KST 예약)에 자동 실행됩니다.
  ⚠️ **GitHub Actions의 예약 실행은 "정확한 시각 보장"이 아니라 "그 시각
  이후 가능한 빨리 실행"** 방식이라, 실행 큐가 몰리는 날에는 몇십 분에서
  길게는 몇 시간까지 늦어질 수 있습니다(GitHub 공식 문서에도 명시된
  한계입니다). 정각(00분)을 피해 47분으로 잡아 혼잡을 최대한 줄였지만,
  100% 정시 실행을 보장하지는 않습니다. 매번 정확히 6시에 딱 맞춰
  실행되어야 한다면, 외부 무료 크론 서비스(cron-job.org 등)로
  GitHub Actions를 호출하는 방식이 훨씬 정확합니다 — 필요하시면 설정을
  도와드릴 수 있습니다.
